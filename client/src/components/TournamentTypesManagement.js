...
const TournamentTypesManagement = () => {
  ...
  const fetchData = async () => {
    try {
      const [typesRes, gamesRes] = await Promise.all([
        api.get('/tournament-types/admin'),
        api.get('/games')
      ]);
      setTournamentTypes(typesRes.data || []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await api.put(`/tournament-types/${editingId}`, formData);
      } else {
        await api.post('/tournament-types', formData);
      }
      fetchData();
      ...
      setError(error.response?.data?.error || 'Failed to save tournament type');
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this tournament type?')) {
      try {
        await api.delete(`/tournament-types/${id}`);
        fetchData();
      } catch (error) {
        alert('Error deleting tournament type');
      }
    }
  };
  ...
  const toggleActive = async (type) => {
    try {
      await api.put(`/tournament-types/${type._id}`, {
        ...type,
        isActive: !type.isActive
      });
      fetchData();
    } catch (error) {
      alert('Error updating tournament type');
    }
  };
  ...
  return (
    <div className="bg-charcoal border border-lava-orange/30 rounded-lg p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-lava-orange">Tournament Types</h2>
        <Button
          variant="primary"
          onClick={() => {
        ...
        >
          {showForm ? 'Cancel' : '+ Add Tournament Type'}
        </Button>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                rows="2"
                className="w-full px-4 py-2 bg-charcoal border border-lava-orange/30 rounded-lg text-off-white focus:outline-none focus:border-lava-orange"
                placeholder="Tournament type description"
              />
            </div>
          </div>
          <div className="flex gap-4">
            <Button type="submit" variant="primary">
              {editingId ? 'Update' : 'Add'} Tournament Type
            </Button>
            {editingId && (
              <Button
                type="button"
      ...
      {tournamentTypes.length === 0 && (
        <div className="text-center py-12 text-gray-400">
          <p>No tournament types found. Add one to get started!</p>
        </div>
      )}
    </div>
  );
